;
(() => {
    const searchStringElement = document.querySelector('#search--searchString');
        ...
        for (const agenda of agendaMetadata) {
          ...
          <div class="level-right">
            <div class="level-item" data-field="agendaDate"></div>
                ...
                panelBlockElement.querySelector('[data-field="agendaTitle"]').textContent = agenda.agendaTitle;
                panelBlockElement.querySelector('[data-field="agendaDate"]').textContent = agenda.agendaDate;
                if ((agenda.outline ?? []).length > 0) {
                    panelBlockElement.insertAdjacentHTML('beforeend', '<ul class="fa-ul"></ul>');
                    for (const [index, outlineItem] of agenda.outline.entries()) {
                        if (index > 10) {
                            panelBlockElement.querySelector('ul').insertAdjacentHTML('beforeend', `<li>
